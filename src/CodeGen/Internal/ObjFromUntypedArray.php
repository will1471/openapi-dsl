...
final class ObjFromUntypedArray
{
    ...
    public function __construct(private Obj $obj, private ParseResult $parseResult, private string $namespace)
    {
    }
    ...
    private function propertyStatements(Variable $inputArray, Prop $prop, Variable $outputArray): array
    {
        ...
        $arrayKeyExists = c::array_key_exists($prop->getName(), $inputArray);

        if (!$prop->isFieldOptional()) {
            $stmts[] = c::if_(c::not_($arrayKeyExists))
                ->then(c::throwException('Property "' . $prop->getName() . '" is required, but missing.'))
                ->build();
        }

        // @todo can remove?
        if (!$prop->isNullable()) {
            $stmts[] = c::if_(c::not_(c::isset_(c::array_dim($inputArray, $prop->getName()))))
                ->then(c::throwException('Property "' . $prop->getName() . '" is not nullable, but found null.'))
                ->build();
        }
        ...
        if ($prop->isList() == false) {
            ...
            $stmts[] = c::assign(c::array_dim($outputArray, $prop->getName()), $tmp);
        }
    ...
    {
        return match (true) {
            $prop->getType() == 'int' || $prop->getType() == 'integer' => 'int',
            $prop->getType() == 'string' => 'string',
            $this->parseResult->hasEnum($prop->getType()) => 'enum',
            $this->parseResult->hasObj($prop->getType()) => 'obj',
    ...
    {
        $value = c::array_dim($inputArray, $prop->getName());

        $isObj = $this->parseResult->hasObj($prop->getType());
        $isEnum = $this->parseResult->hasEnum($prop->getType());
        if ($isObj || $isEnum) {
            $value = c::callStaticMethod($prop->getType(), $isObj ? 'fromUntypedArray' : 'checkString')($value);
        }

        return c::assign(c::array_dim($outputArray, $prop->getName()), $value);
    }
    ...
    private function foreachAssertAndAssignPropToTmp(Variable $inputArray, Prop $prop, Variable $tmp): Foreach_
    {
        ...
        if ($propType == 'enum' || $propType == 'obj') {
            ...
            $assign = c::callStaticMethod($prop->getType(), $method)($item);
        }

        return c::foreach_(c::array_dim($inputArray, $prop->getName()))
            ->as_($item)
            ->withMixedDocBloc()
            ->exec(
    ...
    private function assertFunc(Variable $inputArray, string $funcName, Prop $prop): If_
    {
        ...
        return c::if_(c::not_(c::function($funcName)(c::array_dim($inputArray, $prop->getName()))))
            ->then(
                c::throwException(
                    c::concat(
                        c::str("Property {$prop->getName()} should be $expectedType, but found"),
                        c::function('get_debug_type')(c::array_dim($inputArray, $prop->getName()))
                    )
                )
            )->build();
    }
